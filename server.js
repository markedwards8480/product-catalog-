...
function getHTML() {
    ...
    // Settings modal
    html += '.settings-modal{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);display:none;align-items:center;justify-content:center;z-index:1001;overflow-y:auto;padding:2rem}.settings-modal.active{display:flex}.settings-modal-content{background:white;border-radius:18px;padding:2rem;max-width:900px;width:95%;max-height:90vh;overflow-y:auto;position:relative;box-shadow:0 20px 60px rgba(0,0,0,0.2)}';
    ...
    html += '<div id="mainApp"><header class="header"><h1 style="color:#1e3a5f;font-weight:700;font-size:1.5rem">Mark Edwards Apparel</h1><div class="header-right"><div class="user-menu-wrapper" style="position:relative"><button class="btn btn-secondary" id="userMenuBtn" style="display:flex;align-items:center;gap:0.5rem"><span id="userInfo">Welcome</span> ▾</button><div id="userMenu" class="user-menu hidden"><button class="user-menu-item" id="changePinBtn">Change PIN</button><button class="user-menu-item" id="logoutBtn">Sign Out</button></div></div><button class="btn btn-secondary" id="helpBtn">User Guide</button><button class="btn btn-secondary" id="historyBtn">History</button><button class="btn btn-secondary" id="settingsBtn">Settings</button></div></header>';
    ...
    html += '<div id="settingsModal" class="settings-modal"><div class="settings-modal-content"><button class="modal-close" id="settingsClose">&times;</button><h2>Settings</h2><div class="tabs"><button class="tab active" data-tab="zoho2">Zoho Sync</button><button class="tab" data-tab="import2">Import CSV</button><button class="tab" data-tab="sales2">Import Sales</button><button class="tab" data-tab="autoimport2">Auto Import</button><button class="tab" data-tab="ai2">AI Analysis</button><button class="tab" data-tab="cache2">Image Cache</button><button class="tab" data-tab="users2">Users</button><button class="tab" data-tab="system2">System Health</button><button class="tab" data-tab="merch2">Merchandising</button></div>';
    ...
    html += 'function showApp(displayName,r){document.getElementById("loginPage").style.display="none";document.getElementById("mainApp").style.display="block";document.getElementById("userInfo").textContent=displayName||"User";if(r==="admin"){document.getElementById("settingsBtn").style.display="inline-block"}}';
    ...
    html += 'document.getElementById("historyBtn").addEventListener("click",function(){document.getElementById("historyPanel").classList.toggle("hidden");document.getElementById("settingsModal").classList.remove("active")});';
    html += 'document.getElementById("settingsBtn").addEventListener("click",function(){document.getElementById("settingsModal").classList.add("active");document.getElementById("historyPanel").classList.add("hidden");loadSystemHealth();loadSalesStats()});';
    html += 'document.getElementById("settingsClose").addEventListener("click",function(){document.getElementById("settingsModal").classList.remove("active")});';
    html += 'document.getElementById("settingsModal").addEventListener("click",function(e){if(e.target.id==="settingsModal")document.getElementById("settingsModal").classList.remove("active")});';
