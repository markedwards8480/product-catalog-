...
app.post('/api/order-requests', requireAuth, async function(req, res) {
    try {
        ...
        var result = await pool.query(
            "INSERT INTO order_requests (request_number, detail_id, user_id, user_name, customer_name, product_ids, product_count, cancel_date, notes, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'pending') RETURNING *",
            [requestNumber, detailId, req.session.userId, req.session.userName || 'Unknown',
             ...
             b.cancel_date || null, b.notes || null]
        );
        var order = result.rows[0];
        order.detail_url = '/order/' + detailId;

        // Send email
        sendOrderRequestEmail(order).catch(function(e) { console.error('Order email error:', e); });
...
function getHTML() {
    ...
    html += '<header class="header"><h1 style="color:#1e3a5f;font-weight:700;font-size:1.5rem">Mark Edwards Apparel</h1><div class="header-right"><div class="user-menu-wrapper" style="position:relative"><button class="btn btn-secondary" id="userMenuBtn" style="display:flex;align-items:center;gap:0.5rem"><span id="userInfo">Welcome</span> ▾</button><div id="userMenu" class="user-menu hidden"><button class="user-menu-item" id="changePinBtn">Change PIN</button><button class="user-menu-item" id="logoutBtn">Sign Out</button></div></div><button class="btn btn-secondary" id="helpBtn">User Guide</button><button class="btn btn-secondary" onclick="toggleOrdersList()">Orders</button><button class="btn btn-secondary" id="historyBtn">History</button><button class="btn btn-secondary" id="adminBtn">Admin</button></div></header>';
