...
app.post('/api/users', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        res.json({ success: true });
    } catch (err) { if (err.code === '23505') res.status(400).json({ error: 'Username already exists' }); else res.status(500).json({ error: err.message }); }
});
...
document.getElementById('addUserBtn').addEventListener('click',async()=>{const u=document.getElementById('newUsername').value;const p=document.getElementById('newPassword').value;const r=document.getElementById('newRole').value;if(!u||!p){alert('Please enter username and password');return}try{const res=await fetch('/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p,role:r})});const d=await res.json();if(d.success){document.getElementById('newUsername').value='';document.getElementById('newPassword').value='';loadUsers()}else{alert(d.error||'Failed')}}catch(e){alert('Error: '+e.message)}});
async function deleteUser(id){if(!confirm('Delete this user?'))return;try{const r=await fetch('/api/users/'+id,{method:'DELETE'});const d=await r.json();if(d.success)loadUsers();else alert(d.error||'Failed')}catch(e){alert('Error: '+e.message)}}
checkSession();
</script>
</body>
</html>`;
