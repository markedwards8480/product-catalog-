...
// Get open sales orders quantities by style for Supply vs Demand calculation
app.get('/api/open-orders-by-style', requireAuth, async function(req, res) {
    try {
        var result = await pool.query(`
            SELECT
                base_style,
                SUM(quantity) as total_open_qty
            FROM sales_data
            WHERE document_type = 'Sales Order'
            AND (status = 'Open' OR status = 'open' OR status = 'OPEN')
            GROUP BY base_style
        `);
        ...
        result.rows.forEach(function(row) {
            if (row.base_style) {
                openOrdersByStyle[row.base_style] = parseInt(row.total_open_qty) || 0;
            }
        });

        console.log('Loaded open orders for', Object.keys(openOrdersByStyle).length, 'styles');
        res.json({ success: true, openOrders: openOrdersByStyle });
    } catch (err) {
...
function getHTML() {
    ...
    html += '<div class="stats"><div><div class="stat-value" id="totalStyles">0</div><div class="stat-label">Styles</div></div><div id="availNowStat" class="stat-box"><div class="stat-value" id="totalAvailNow">0</div><div class="stat-label">Avail Now</div></div><div id="leftToSellStat" class="stat-box stat-active"><div class="stat-value" id="totalLeftToSell">0</div><div class="stat-label">Left to Sell</div></div>' + (SUPPLY_DEMAND_FEATURE_ENABLED ? '<div id="availToSellStat" class="stat-box" style="display:none"><div class="stat-value" id="totalAvailToSell" style="color:#1e3a5f">0</div><div class="stat-label">Avail to Sell</div></div>' : '') + '<div class="qty-toggle"><button class="qty-toggle-btn" id="toggleAvailableNow" data-mode="available_now">Available Now</button><button class="qty-toggle-btn active" id="toggleLeftToSell" data-mode="left_to_sell">Left to Sell</button>' + (SUPPLY_DEMAND_FEATURE_ENABLED ? '<div style="display:flex;align-items:center;gap:0.5rem;margin-left:1rem;padding-left:1rem;border-left:1px solid #ddd"><input type="checkbox" id="supplyDemandToggle" style="cursor:pointer"><label for="supplyDemandToggle" style="cursor:pointer;font-size:0.75rem;white-space:nowrap">Supply vs Demand</label></div>' : '') + '</div><div style="margin-left:auto;text-align:right;font-size:0.7rem;color:#999"><span id="dataFreshness">Loading...</span></div></div>';
    ...
    html += 'async function fetchOpenOrders(){try{var resp=await fetch("/api/open-orders-by-style");var data=await resp.json();if(data.success){openOrdersByStyle=data.openOrders;console.log("Loaded open orders for",Object.keys(openOrdersByStyle).length,"styles")}else{console.error("Error loading open orders:",data.error)}}catch(err){console.error("Error fetching open orders:",err)}}';
    ...
    html += 'function showProductModal(id){currentModalProductId=id;var pr=products.find(function(p){return p.id===id});if(!pr)return;var imgUrl=getImageUrl(pr.image_url);document.getElementById("modalImage").src=imgUrl||"";document.getElementById("modalStyle").textContent=pr.style_id;document.getElementById("modalName").textContent=pr.name;var cols=pr.colors||[];var colorName=cols.length===1?cols[0].color_name:(pr.category||"");document.getElementById("modalCategory").textContent=colorName;var totNow=0,totLts=0;cols.forEach(function(c){var aNow=c.available_now||c.available_qty||0;var lts=c.left_to_sell||0;totNow+=aNow;totLts+=lts});var ch="";if(cols.length>1){ch="<table style=\\"width:100%;border-collapse:collapse;font-size:0.875rem\\"><thead><tr style=\\"border-bottom:1px solid #ddd\\"><th style=\\"text-align:left;padding:0.5rem 0;font-weight:600;color:#666\\">Color</th><th style=\\"text-align:right;padding:0.5rem 0;font-weight:600;color:#666;width:80px\\">Avail Now</th><th style=\\"text-align:right;padding:0.5rem 0;font-weight:600;color:#666;width:80px\\">Left to Sell</th></tr></thead><tbody>";cols.forEach(function(c){var aNow=c.available_now||c.available_qty||0;var lts=c.left_to_sell||0;ch+="<tr><td style=\\"padding:0.4rem 0\\">"+c.color_name+"</td><td style=\\"text-align:right;padding:0.4rem 0\\">"+aNow.toLocaleString()+"</td><td style=\\"text-align:right;padding:0.4rem 0;color:#666\\">"+lts.toLocaleString()+"</td></tr>"});ch+="</tbody></table>"}document.getElementById("modalColors").innerHTML=ch;var modalTotalHtml="<span style=\\"margin-right:2rem\\">Now: "+totNow.toLocaleString()+"</span><span>LTS: "+totLts.toLocaleString()+"</span>";if(supplyDemandMode){var baseStyle=pr.style_id.split("-")[0];var openOrders=openOrdersByStyle[baseStyle]||0;var totalOnOrder=totNow+totLts;var availToSell=totalOnOrder-openOrders;var availColor=availToSell<0?"#ff3b30":"#1e3a5f";modalTotalHtml+="<div style=\\"margin-top:1rem;padding:0.75rem;background:#f8f9fa;border-radius:8px;font-size:0.875rem\\"><div style=\\"display:flex;justify-content:space-between;padding:0.4rem 0\\"><span>Total On Order:</span><span style=\\"font-weight:500\\">"+totalOnOrder.toLocaleString()+"</span></div><div style=\\"display:flex;justify-content:space-between;padding:0.4rem 0;border-bottom:1px solid #e0e0e0\\"><span>Open Sales Orders:</span><span style=\\"font-weight:500\\">"+openOrders.toLocaleString()+"</span></div><div style=\\"display:flex;justify-content:space-between;padding:0.5rem 0 0.25rem;font-weight:700;font-size:0.95rem;color:"+availColor+"\\"><span>Available to Sell:</span><span>"+availToSell.toLocaleString()+"</span></div></div>"}document.getElementById("modalTotal").innerHTML=modalTotalHtml;document.getElementById("modalNote").value=userNotes[id]||"";document.getElementById("modalPickBtn").style.display="";var isPicked=userPicks.indexOf(id)!==-1;document.getElementById("modalPickBtn").textContent=isPicked?"♥ In My Picks":"♡ Add to My Picks";document.getElementById("modal").classList.add("active");loadSalesHistory(pr.style_id)}';
