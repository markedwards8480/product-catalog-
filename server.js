...
async function initDB() {
    try {
        ...
        await pool.query('CREATE TABLE IF NOT EXISTS selections (id SERIAL PRIMARY KEY, share_id VARCHAR(50) UNIQUE NOT NULL, name VARCHAR(255), product_ids INTEGER[], created_by VARCHAR(255), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)');
...
app.post('/api/selections', requireAuth, async function(req, res) {
    try {
        ...
        await pool.query('INSERT INTO selections (share_id, name, product_ids, created_by) VALUES ($1, $2, $3, $4)', [shareId, name, productIds, req.session.username || 'anonymous']);
...
function getPDFHTML(selection, products) {
    ...
    html += '</div><div class="footer">Product availability subject to change</div></body></html>';
    return html;
}
...
function getHTML() {
    ...
    html += '.tabs{display:flex;gap:0.5rem;margin-bottom:1rem}.tab{padding:0.5rem 1rem;border:none;background:#eee;cursor:pointer;border-radius:4px}.tab.active{background:#2c5545;color:white}.tab-content{display:none}.tab-content.active{display:block}';
    ...
    html += '.modal{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);display:none;align-items:center;justify-content:center;z-index:1000}.modal.active{display:flex}.modal-content{background:white;border-radius:8px;max-width:800px;width:90%;max-height:90vh;overflow:auto;position:relative}.modal-body{display:flex}.modal-image{width:50%;background:#f0f0f0;min-height:300px;display:flex;align-items:center;justify-content:center}.modal-image img{max-width:100%;max-height:400px;object-fit:contain}.modal-details{width:50%;padding:2rem}.modal-close{position:absolute;top:1rem;right:1rem;background:white;border:none;font-size:1.5rem;cursor:pointer;border-radius:50%;width:36px;height:36px}';
    ...
    html += '.view-controls{display:flex;align-items:center;gap:1rem;margin-bottom:1rem;padding:0.75rem 1rem;background:white;border-radius:8px}.view-controls label{font-weight:500;color:#333}';
    ...
    html += '<div id="loginPage" class="login-page"><div class="login-box"><h1>Product Catalog</h1><form id="loginForm"><div class="form-group"><label>Username</label><input type="text" id="username" required></div><div class="form-group"><label>Password</label><input type="password" id="password" required></div><button type="submit" class="btn btn-primary" style="width:100%">Sign In</button><div id="loginError" class="error hidden"></div></form></div></div>';
    
    html += '<div id="mainApp" class="hidden"><header class="header"><h1>Product Catalog</h1><div class="search-box"><input type="text" id="searchInput" placeholder="Search products..."></div><div class="header-right"><span id="userInfo"></span><button class="btn btn-secondary" id="adminBtn" style="display:none">Admin</button><button class="btn btn-secondary" id="logoutBtn">Sign Out</button></div></header>';
    
    html += '<main class="main"><div id="adminPanel" class="admin-panel hidden"><h2>Admin Panel</h2><div class="tabs"><button class="tab active" data-tab="zoho">Zoho Sync</button><button class="tab" data-tab="import">Import CSV</button><button class="tab" data-tab="users">Users</button><button class="tab" data-tab="history">History</button></div>';
    ...
    html += '<div class="view-controls"><label>Tile Size:</label><button class="size-btn" data-size="small">Small</button><button class="size-btn active" data-size="medium">Medium</button><button class="size-btn" data-size="large">Large</button><span style="margin-left:auto;color:#666;font-size:0.875rem">Click products to select for sharing</span></div>';
    ...
    // Selection bar
    html += '<div class="selection-bar" id="selectionBar"><span class="selection-count"><span id="selectedCount">0</span> items selected</span><div class="selection-actions"><button class="btn btn-secondary" id="clearSelectionBtn">Clear</button><button class="btn btn-primary" id="shareSelectionBtn">Share / Download</button></div></div>';
    
    // Share modal
    html += '<div class="share-modal" id="shareModal"><div class="share-modal-content"><h3>Share Selection</h3><input type="text" id="selectionName" placeholder="Name this selection (e.g. Spring Collection for Acme Co)"><div class="share-modal-actions"><button class="btn btn-secondary" id="cancelShareBtn">Cancel</button><button class="btn btn-primary" id="createShareBtn">Create Link</button></div><div class="share-result hidden" id="shareResult"><p><strong>Share Link:</strong></p><p><a id="shareLink" href="" target="_blank"></a></p><p style="margin-top:0.5rem"><a id="pdfLink" href="" target="_blank">Download PDF</a></p></div></div></div>';
    ...
    html += '<script>';
    html += 'var products=[];var currentFilter="all";var currentSize="medium";var selectedProducts=[];';
    
    html += 'function checkSession(){fetch("/api/session").then(function(r){return r.json()}).then(function(d){if(d.loggedIn){showApp(d.username,d.role);loadProducts();loadZohoStatus();if(d.role==="admin"){loadUsers();loadHistory()}}})}';
    ...
    html += 'function toggleSelect(id,e){e.stopPropagation();var idx=selectedProducts.indexOf(id);if(idx===-1){selectedProducts.push(id)}else{selectedProducts.splice(idx,1)}updateSelectionUI();renderProducts()}';
    
    html += 'function updateSelectionUI(){document.getElementById("selectedCount").textContent=selectedProducts.length;var bar=document.getElementById("selectionBar");if(selectedProducts.length>0){bar.classList.add("visible")}else{bar.classList.remove("visible")}}';
    
    html += 'function renderProducts(){var s=document.getElementById("searchInput").value.toLowerCase();var f=products.filter(function(p){var ms=!s||p.style_id.toLowerCase().indexOf(s)!==-1||p.name.toLowerCase().indexOf(s)!==-1;var mc=currentFilter==="all"||p.category===currentFilter;return ms&&mc});if(f.length===0){document.getElementById("productGrid").innerHTML="";document.getElementById("emptyState").classList.remove("hidden")}else{document.getElementById("emptyState").classList.add("hidden");var h="";f.forEach(function(pr){var cols=pr.colors||[];var tot=0;cols.forEach(function(c){tot+=c.available_qty||0});var ch="";var mx=Math.min(cols.length,3);for(var d=0;d<mx;d++){ch+="<div class=\\"color-row\\"><span>"+cols[d].color_name+"</span><span>"+(cols[d].available_qty||0).toLocaleString()+"</span></div>"}if(cols.length>3)ch+="<div class=\\"color-row\\" style=\\"color:#999\\">+"+(cols.length-3)+" more</div>";var imgUrl=getImageUrl(pr.image_url);var im=imgUrl?"<img src=\\""+imgUrl+"\\" onerror=\\"this.parentElement.innerHTML=\'No Image\'\\">":"No Image";var sel=selectedProducts.indexOf(pr.id)!==-1?"selected":"";h+="<div class=\\"product-card "+sel+"\\" onclick=\\"toggleSelect("+pr.id+",event)\\"><div class=\\"select-badge\\">✓</div><div class=\\"product-image\\">"+im+"</div><div class=\\"product-info\\"><div class=\\"product-style\\">"+pr.style_id+"</div><div class=\\"product-name\\">"+pr.name+"</div><div class=\\"color-list\\">"+ch+"</div><div class=\\"total-row\\"><span>Total</span><span>"+tot.toLocaleString()+"</span></div></div></div>"});document.getElementById("productGrid").innerHTML=h}document.getElementById("totalStyles").textContent=f.length;var tu=0;f.forEach(function(p){(p.colors||[]).forEach(function(c){tu+=c.available_qty||0})});document.getElementById("totalUnits").textContent=tu.toLocaleString()}';
