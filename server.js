...
function getHTML() {
    ...
    html += '.header{background:linear-gradient(135deg,#1e3a5f 0%,#2d5a7f 100%);padding:0 1.5rem;height:60px;display:flex;justify-content:space-between;align-items:center;position:fixed;top:0;left:0;right:0;z-index:100}.header h1{font-size:1.25rem;font-weight:600;color:white;letter-spacing:-0.01em}.header-right{display:flex;gap:1rem;align-items:center}';
    ...
    html += '.app-layout{display:flex;margin-top:60px;min-height:calc(100vh - 60px)}.sidebar{width:280px;background:white;border-right:1px solid #e5e5e5;position:fixed;top:60px;left:0;bottom:0;overflow-y:auto;padding:1rem;z-index:80}.sidebar-section{margin-bottom:1.25rem;padding-bottom:1rem;border-bottom:1px solid #eee}.sidebar-section:last-child{border-bottom:none}.sidebar-section h3{font-size:0.7rem;text-transform:uppercase;letter-spacing:0.5px;color:#86868b;margin-bottom:0.625rem;font-weight:600}.content-area{margin-left:280px;flex:1;min-height:100%}';
    html += '.stats-compact{display:flex;gap:1rem;padding:0.75rem;background:#f5f5f7;border-radius:10px;margin-bottom:1rem}.stats-compact .stat{text-align:center;flex:1}.stats-compact .stat-value{font-size:1.125rem;font-weight:700;color:#1e3a5f}.stats-compact .stat-value.muted{color:#86868b;font-weight:400}.stats-compact .stat-label{font-size:0.625rem;text-transform:uppercase;color:#86868b;letter-spacing:0.02em}';
    html += '.qty-toggle-sidebar{display:flex;gap:0.25rem;margin-bottom:1rem}.qty-toggle-sidebar button{flex:1;padding:0.5rem 0.25rem;border:1px solid #d2d2d7;background:white;font-size:0.75rem;cursor:pointer;transition:all 0.15s;font-weight:500;color:#1e3a5f}.qty-toggle-sidebar button:first-child{border-radius:8px 0 0 8px}.qty-toggle-sidebar button:last-child{border-radius:0 8px 8px 0}.qty-toggle-sidebar button.active{background:#1e3a5f;color:white;border-color:#1e3a5f}';
    html += '.quick-filters{display:flex;gap:0.375rem;margin-bottom:1rem;flex-wrap:wrap}.quick-filter-btn{padding:0.5rem 0.75rem;border:1px solid #d2d2d7;background:white;border-radius:8px;font-size:0.75rem;cursor:pointer;transition:all 0.15s;font-weight:500;color:#1e3a5f}.quick-filter-btn:hover{border-color:#1e3a5f}.quick-filter-btn.active{background:#dbeafe;border-color:#3b82f6;color:#1e40af}';
    html += '.search-box-sidebar{position:relative;margin-bottom:0.5rem}.search-box-sidebar input{width:100%;padding:0.625rem 0.875rem;padding-right:2rem;border:1px solid #d2d2d7;border-radius:8px;font-size:0.875rem;transition:border-color 0.2s}.search-box-sidebar input:focus{outline:none;border-color:#1e3a5f}.search-box-sidebar .clear-search{position:absolute;right:8px;top:50%;transform:translateY(-50%);background:#e5e5e5;border:none;border-radius:50%;width:18px;height:18px;cursor:pointer;font-size:0.7rem;color:#666;display:none}.search-box-sidebar .clear-search.visible{display:block}.ai-search-badge{font-size:0.65rem;color:#0088c2;margin-top:0.25rem}';
    html += '.category-list{display:flex;flex-direction:column;gap:0.125rem;max-height:280px;overflow-y:auto}.category-item{padding:0.5rem 0.75rem;border-radius:6px;cursor:pointer;font-size:0.8125rem;transition:background 0.15s;display:flex;justify-content:space-between;color:#1e3a5f}.category-item:hover{background:#f5f5f7}.category-item.active{background:#1e3a5f;color:white}.category-count{font-size:0.7rem;opacity:0.6}';
    html += '.color-chips{display:flex;flex-wrap:wrap;gap:0.375rem}.color-chip{padding:0.375rem 0.625rem;background:#f5f5f7;border-radius:16px;font-size:0.75rem;cursor:pointer;transition:all 0.15s;color:#1e3a5f}.color-chip:hover{background:#e5e5e5}.color-chip.active{background:#1e3a5f;color:white}';
    html += '.qty-range-inputs{display:flex;gap:0.5rem;align-items:center}.qty-range-inputs input{width:70px;padding:0.5rem;border:1px solid #d2d2d7;border-radius:6px;font-size:0.8125rem;text-align:center}.qty-range-inputs input:focus{outline:none;border-color:#1e3a5f}.qty-range-inputs span{color:#86868b}';
    html += '.view-options-grid{display:flex;flex-direction:column;gap:0.75rem}.view-option-row{display:flex;justify-content:space-between;align-items:center}.view-option-label{font-size:0.8125rem;color:#666}.view-option-btns{display:flex;gap:0.25rem}.view-option-btns button{padding:0.375rem 0.5rem;border:1px solid #d2d2d7;background:white;font-size:0.7rem;cursor:pointer;border-radius:4px;transition:all 0.15s;color:#1e3a5f}.view-option-btns button:hover{border-color:#1e3a5f}.view-option-btns button.active{background:#1e3a5f;color:white;border-color:#1e3a5f}';
    html += '.sort-select-sidebar{width:100%;padding:0.5rem 0.75rem;border:1px solid #d2d2d7;border-radius:8px;font-size:0.8125rem;background:white;color:#1e3a5f;cursor:pointer}.sort-select-sidebar:focus{outline:none;border-color:#1e3a5f}';
    html += '.reset-filters-btn{width:100%;padding:0.75rem;background:#fee2e2;color:#dc2626;border:none;border-radius:8px;font-size:0.8125rem;font-weight:500;cursor:pointer;transition:background 0.2s;margin-top:0.5rem}.reset-filters-btn:hover{background:#fecaca}';
    html += '.sidebar-footer{font-size:0.7rem;color:#86868b;text-align:center;padding-top:0.75rem;border-top:1px solid #eee;margin-top:0.75rem}';
    html += '.content-header{display:flex;justify-content:space-between;align-items:center;padding:1rem 1.5rem;background:white;border-bottom:1px solid #eee;position:sticky;top:60px;z-index:70}.results-info{font-size:0.875rem;color:#666}.share-btn-header{padding:0.5rem 1rem;background:white;border:1px solid #1e3a5f;color:#1e3a5f;border-radius:8px;font-size:0.8125rem;cursor:pointer;transition:all 0.2s;font-weight:500}.share-btn-header:hover{background:#1e3a5f;color:white}';
    ...
    // Legacy view controls - keeping for compatibility but hidden in sidebar layout
    html += '.view-controls{display:none}.view-controls.compact{display:none}.view-controls label{font-weight:500;color:#6e6e73;font-size:0.8125rem}';
    ...
    html += '<div id="mainApp" class="hidden"><header class="header"><h1>Mark Edwards Apparel</h1><div class="header-right"><div class="user-menu-wrapper" style="position:relative"><button class="btn btn-secondary" id="userMenuBtn" style="display:flex;align-items:center;gap:0.5rem;background:rgba(255,255,255,0.15);border:none;color:white"><span id="userInfo">Welcome</span> ▾</button><div id="userMenu" class="user-menu hidden"><button class="user-menu-item" id="changePinBtn">Change PIN</button><button class="user-menu-item" id="logoutBtn">Sign Out</button></div></div><button class="btn btn-secondary" id="helpBtn" style="background:rgba(255,255,255,0.15);border:none;color:white">Help</button><button class="btn btn-secondary" id="historyBtn" style="background:rgba(255,255,255,0.15);border:none;color:white">History</button><button class="btn btn-secondary" id="adminBtn" style="display:none;background:rgba(255,255,255,0.15);border:none;color:white">Admin</button></div></header>';
    
    html += '<div class="app-layout">';
    
    // Sidebar
    html += '<aside class="sidebar" id="sidebar">';
    html += '<div class="stats-compact"><div class="stat"><div class="stat-value" id="totalStyles">0</div><div class="stat-label">Styles</div></div><div class="stat"><div class="stat-value muted" id="totalAvailNow">0</div><div class="stat-label">Avail Now</div></div><div class="stat"><div class="stat-value" id="totalLeftToSell">0</div><div class="stat-label">Left to Sell</div></div></div>';
    html += '<div class="qty-toggle-sidebar"><button class="qty-toggle-btn" id="toggleAvailableNow" data-mode="available_now">Available Now</button><button class="qty-toggle-btn active" id="toggleLeftToSell" data-mode="left_to_sell">Left to Sell</button></div>';
    html += '<div class="quick-filters"><button class="quick-filter-btn" data-special="new">New Arrivals</button><button class="quick-filter-btn" data-special="picks">My Picks</button><button class="quick-filter-btn" data-special="notes">Has Notes</button></div>';
    html += '<div class="sidebar-section"><h3>Search</h3><div class="search-box-sidebar"><input type="text" id="searchInput" placeholder="Search products..."><button class="clear-search" id="clearSearchBtn">&times;</button></div><div class="ai-search-badge" id="aiSearchIndicator" style="display:none">✨ AI-enhanced search</div></div>';
    html += '<div class="sidebar-section"><h3>Category</h3><div class="category-list" id="categoryFilters"></div></div>';
    html += '<div class="sidebar-section"><h3>Color</h3><div class="color-chips" id="colorChips"><span class="color-chip active" data-color="">All</span></div></div>';
    html += '<div class="sidebar-section"><h3>Quantity Range</h3><div class="qty-range-inputs"><input type="number" id="minQty" placeholder="Min"><span>–</span><input type="number" id="maxQty" placeholder="Max"></div></div>';
    html += '<div class="sidebar-section"><h3>View Options</h3><div class="view-options-grid"><div class="view-option-row"><span class="view-option-label">Card Size</span><div class="view-option-btns" id="sizeButtons"><button class="size-btn" data-size="list">List</button><button class="size-btn" data-size="small">S</button><button class="size-btn active" data-size="medium">M</button><button class="size-btn" data-size="large">L</button></div></div><div class="view-option-row"><span class="view-option-label">Grouping</span><div class="view-option-btns"><button class="group-btn active" id="groupByStyleToggle" data-grouped="true">By Style</button><button class="group-btn" data-grouped="false">All Colors</button></div></div></div></div>';
    html += '<div class="sidebar-section"><h3>Sort By</h3><select class="sort-select-sidebar" id="sortSelect"><option value="qty-high" selected>Quantity: High to Low</option><option value="qty-low">Quantity: Low to High</option><option value="name-asc">Name: A to Z</option><option value="name-desc">Name: Z to A</option><option value="newest">Newest First</option></select></div>';
    html += '<button class="reset-filters-btn" id="resetAllFilters">↺ Reset All Filters</button>';
    html += '<div class="sidebar-footer"><span id="dataFreshness">Loading...</span></div>';
    html += '</aside>';
    
    // Content area
    html += '<main class="content-area">';
    html += '<div class="content-header"><span class="results-info" id="resultsInfo">Loading...</span><button class="share-btn-header" id="selectModeBtn">📤 Select for Sharing</button></div>';
    ...
    // Group by style toggle (new sidebar version)
    html += 'document.querySelectorAll(".group-btn").forEach(function(btn){btn.addEventListener("click",function(e){document.querySelectorAll(".group-btn").forEach(function(b){b.classList.remove("active")});e.target.classList.add("active");groupByStyle=e.target.getAttribute("data-grouped")==="true";renderProducts()})});';
    
    // Reset all filters button
    html += 'document.getElementById("resetAllFilters").addEventListener("click",function(){document.getElementById("searchInput").value="";document.getElementById("minQty").value="";document.getElementById("maxQty").value="";currentFilter="all";colorFilter=null;specialFilter=null;minColorsFilter=0;customerStyleFilter=null;currentSort="qty-high";document.getElementById("sortSelect").value="qty-high";document.querySelectorAll(".category-item").forEach(function(c){c.classList.remove("active")});document.querySelector(".category-item[data-cat=\\"all\\"]").classList.add("active");document.querySelectorAll(".color-chip").forEach(function(c){c.classList.remove("active")});document.querySelector(".color-chip[data-color=\\"\\"]").classList.add("active");document.querySelectorAll(".quick-filter-btn").forEach(function(b){b.classList.remove("active")});renderProducts()});';
    
    // Search input with clear button
    html += 'document.getElementById("searchInput").addEventListener("input",function(){var val=this.value;document.getElementById("clearSearchBtn").classList.toggle("visible",val.length>0);renderProducts()});';
    html += 'document.getElementById("clearSearchBtn").addEventListener("click",function(){document.getElementById("searchInput").value="";this.classList.remove("visible");renderProducts()});';
    
    // Update results info when rendering
    html += 'function updateResultsInfo(count,grouped){var info=document.getElementById("resultsInfo");if(info){info.textContent="Showing "+count+" "+(grouped?"groups":"products")}}';
    ...
    html += 'function renderFilters(){var cats=[];var colors={};var catCounts={};allProducts.forEach(function(p){if(p.category){if(cats.indexOf(p.category)===-1)cats.push(p.category);catCounts[p.category]=(catCounts[p.category]||0)+1}(p.colors||[]).forEach(function(c){if(c.color_name)colors[c.color_name]=true})});cats.sort();var h="<div class=\\"category-item active\\" data-cat=\\"all\\"><span>All</span><span class=\\"category-count\\">"+allProducts.length+"</span></div>";cats.forEach(function(c){h+="<div class=\\"category-item\\" data-cat=\\""+c+"\\"><span>"+c+"</span><span class=\\"category-count\\">"+(catCounts[c]||0)+"</span></div>"});document.getElementById("categoryFilters").innerHTML=h;var colorList=Object.keys(colors).sort();var colorChipsEl=document.getElementById("colorChips");if(colorChipsEl){var ch="<span class=\\"color-chip active\\" data-color=\\"\\">All</span>";colorList.forEach(function(c){ch+="<span class=\\"color-chip\\" data-color=\\""+c+"\\">"+c+"</span>"});colorChipsEl.innerHTML=ch;document.querySelectorAll(".color-chip").forEach(function(chip){chip.addEventListener("click",function(e){document.querySelectorAll(".color-chip").forEach(function(c){c.classList.remove("active")});e.target.classList.add("active");colorFilter=e.target.getAttribute("data-color")||null;renderProducts()})})}document.querySelectorAll("[data-cat]").forEach(function(btn){btn.addEventListener("click",function(e){document.querySelectorAll("[data-cat]").forEach(function(b){b.classList.remove("active")});e.target.closest(".category-item").classList.add("active");currentFilter=e.target.closest(".category-item").getAttribute("data-cat");renderProducts()})});document.querySelectorAll(".quick-filter-btn").forEach(function(btn){btn.addEventListener("click",function(e){var sp=e.target.getAttribute("data-special");if(specialFilter===sp){specialFilter=null;e.target.classList.remove("active")}else{document.querySelectorAll(".quick-filter-btn").forEach(function(b){b.classList.remove("active")});specialFilter=sp;e.target.classList.add("active")}renderProducts()})})}';
    ...
    // Sidebar layout - no scroll compacting needed
    html += '// Header is fixed, no compacting needed';
