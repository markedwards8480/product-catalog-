...
function getHTML() {
    ...
    html += '<div class="filters" style="margin-top:0.5rem"><span style="font-weight:500;margin-right:0.5rem">Colors:</span><span id="colorFilters"></span></div>';
    ...
    // Share modal
    html += '<div class="share-modal" id="shareModal"><div class="share-modal-content"><h3>Share Selection</h3><div id="shareForm"><input type="text" id="selectionName" placeholder="Name this selection (e.g. Spring Collection for Acme Co)"><div class="share-modal-actions"><button class="btn btn-secondary" id="cancelShareBtn">Cancel</button><button class="btn btn-primary" id="createShareBtn">Create Link</button></div></div><div class="share-result hidden" id="shareResult"><p style="margin-bottom:0.5rem"><strong>Share Link:</strong></p><div style="display:flex;gap:0.5rem;margin-bottom:1rem"><input type="text" id="shareLinkInput" readonly style="flex:1;padding:0.5rem;border:1px solid #ddd;border-radius:4px;font-size:0.875rem"><button class="btn btn-secondary btn-sm" id="copyLinkBtn">Copy</button></div><div style="display:flex;gap:0.5rem;flex-wrap:wrap"><a class="btn btn-primary" id="pdfLink" href="" target="_blank" style="flex:1;text-align:center">📄 Download PDF</a><a class="btn btn-secondary" id="shareLink" href="" target="_blank" style="flex:1;text-align:center">🔗 Open Share Page</a></div><div style="margin-top:1.5rem;text-align:center"><button class="btn btn-secondary" id="closeShareModalBtn">Done</button></div></div></div></div>';
    ...
    html += 'function renderFilters(){var cats=[];var colors={};allProducts.forEach(function(p){if(p.category&&cats.indexOf(p.category)===-1)cats.push(p.category);(p.colors||[]).forEach(function(c){if(c.color_name)colors[c.color_name]=true})});cats.sort();var h="<button class=\\"filter-btn active\\" data-cat=\\"all\\">All</button>";cats.forEach(function(c){h+="<button class=\\"filter-btn\\" data-cat=\\""+c+"\\">"+c+"</button>"});document.getElementById("categoryFilters").innerHTML=h;var colorList=Object.keys(colors).sort();var ch="<button class=\\"filter-btn active\\" data-color=\\"all\\">All</button>";colorList.slice(0,15).forEach(function(c){ch+="<button class=\\"filter-btn\\" data-color=\\""+c+"\\">"+c+"</button>"});if(colorList.length>15)ch+="<span style=\\"color:#999;font-size:0.875rem\\">+"+(colorList.length-15)+" more</span>";document.getElementById("colorFilters").innerHTML=ch;document.querySelectorAll("[data-cat]").forEach(function(btn){btn.addEventListener("click",function(e){document.querySelectorAll("[data-cat]").forEach(function(b){b.classList.remove("active")});e.target.classList.add("active");currentFilter=e.target.getAttribute("data-cat");renderProducts()})});document.querySelectorAll("[data-color]").forEach(function(btn){btn.addEventListener("click",function(e){document.querySelectorAll("[data-color]").forEach(function(b){b.classList.remove("active")});e.target.classList.add("active");colorFilter=e.target.getAttribute("data-color");if(colorFilter==="all")colorFilter=null;renderProducts()})});document.querySelectorAll("[data-special]").forEach(function(btn){btn.addEventListener("click",function(e){var sp=e.target.getAttribute("data-special");if(specialFilter===sp){specialFilter=null;e.target.classList.remove("active")}else{document.querySelectorAll("[data-special]").forEach(function(b){b.classList.remove("active")});specialFilter=sp;e.target.classList.add("active")}renderProducts()})})}';
    ...
    html += 'document.getElementById("createShareBtn").addEventListener("click",function(){var name=document.getElementById("selectionName").value||"Product Selection";fetch("/api/selections",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productIds:selectedProducts,name:name,shareType:"link"})}).then(function(r){return r.json()}).then(function(d){if(d.success){currentShareId=d.shareId;var url=window.location.origin+"/share/"+d.shareId;document.getElementById("shareLinkInput").value=url;document.getElementById("shareLink").href=url;document.getElementById("pdfLink").href="/api/selections/"+d.shareId+"/pdf";document.getElementById("shareForm").classList.add("hidden");document.getElementById("shareResult").classList.remove("hidden");loadShares()}else{alert(d.error)}})});';
    
    html += 'document.getElementById("copyLinkBtn").addEventListener("click",function(){var input=document.getElementById("shareLinkInput");input.select();document.execCommand("copy");this.textContent="Copied!";var btn=this;setTimeout(function(){btn.textContent="Copy"},2000)});';
