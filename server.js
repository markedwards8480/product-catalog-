...
function getHTML() {
    ...
    html += 'function showGroupModal(baseStyle,variants){var totNow=0,totLts=0,totOnHand=0,totToCome=0;variants.forEach(function(v){(v.colors||[]).forEach(function(c){totNow+=(c.available_now||c.available_qty||0);totLts+=(c.left_to_sell||0);totOnHand+=(c.on_hand||0);totToCome+=(c.to_come||0)})});document.getElementById("modalStyle").textContent=baseStyle;document.getElementById("modalName").textContent=variants[0].name.replace(variants[0].style_id,baseStyle);document.getElementById("modalCategory").textContent=variants[0].category||"";var imgUrl=getImageUrl(variants[0].image_url);document.getElementById("modalImage").src=imgUrl||"";var swatchHtml="<div style=\\"display:flex;gap:0.5rem;flex-wrap:wrap;margin-bottom:1rem\\">";variants.forEach(function(v,i){var vImg=getImageUrl(v.image_url);var colorCode=v.style_id.split("-")[1]||"";var colorName=(v.colors&&v.colors[0])?v.colors[0].color_name:colorCode;swatchHtml+="<div class=\\"color-swatch"+(i===0?" active":"")+"\\" data-idx=\\""+i+"\\" style=\\"padding:0.5rem 0.75rem;border:2px solid "+(i===0?"#1a3b5d":"#ddd")+";border-radius:4px;cursor:pointer;font-size:0.75rem;background:"+(i===0?"#f0f4f8":"#fff")+"\\" onclick=\\"switchVariantImage("+i+",\'"+baseStyle+"\')\\">"+colorName+"</div>"});swatchHtml+="</div>";var ch="<table style=\\"width:100%;border-collapse:collapse;font-size:0.875rem\\"><thead><tr style=\\"border-bottom:1px solid #ddd\\"><th style=\\"text-align:left;padding:0.5rem 0;font-weight:600;color:#666\\">Color Variant</th><th style=\\"text-align:right;padding:0.5rem 0;font-weight:600;color:#666;width:80px\\">Avail Now</th><th style=\\"text-align:right;padding:0.5rem 0;font-weight:600;color:#666;width:80px\\">Left to Sell</th></tr></thead><tbody>";variants.forEach(function(v){var vNow=0,vLts=0;(v.colors||[]).forEach(function(c){vNow+=(c.available_now||c.available_qty||0);vLts+=(c.left_to_sell||0)});var colorName=(v.colors&&v.colors[0])?v.colors[0].color_name:v.style_id;ch+="<tr><td style=\\"padding:0.4rem 0\\">"+colorName+"</td><td style=\\"text-align:right;padding:0.4rem 0\\">"+vNow.toLocaleString()+"</td><td style=\\"text-align:right;padding:0.4rem 0;color:#666\\">"+vLts.toLocaleString()+"</td></tr>"});ch+="</tbody></table>";document.getElementById("modalColors").innerHTML=swatchHtml+ch;var modalTotalHtml="<span style=\\"margin-right:2rem\\">Now: "+totNow.toLocaleString()+"</span><span>LTS: "+totLts.toLocaleString()+"</span>";if(supplyDemandMode){var openOrders=openOrdersByStyle[baseStyle]||0;var totalSupply=totOnHand+totToCome;var availToSell=totalSupply-openOrders;var availColor=availToSell<0?"#ff3b30":"#1e3a5f";modalTotalHtml+="<div style=\\"margin-top:1rem;padding:0.75rem;background:#f8f9fa;border-radius:8px;font-size:0.875rem\\"><div style=\\"display:flex;justify-content:space-between;padding:0.4rem 0\\"><span>On Hand:</span><span style=\\"font-weight:500\\">"+totOnHand.toLocaleString()+"</span></div><div style=\\"display:flex;justify-content:space-between;padding:0.4rem 0\\"><span>To Come (Open POs):</span><span style=\\"font-weight:500\\">"+totToCome.toLocaleString()+"</span></div><div style=\\"display:flex;justify-content:space-between;padding:0.4rem 0;font-weight:600;border-bottom:1px solid #e0e0e0\\"><span>Total Supply:</span><span>"+totalSupply.toLocaleString()+"</span></div><div style=\\"display:flex;justify-content:space-between;padding:0.4rem 0;border-bottom:1px solid #e0e0e0\\"><span>Demand (Open SOs):</span><span style=\\"font-weight:500\\">"+openOrders.toLocaleString()+"</span></div><div style=\\"display:flex;justify-content:space-between;padding:0.5rem 0 0.25rem;font-weight:700;font-size:0.95rem;color:"+availColor+"\\"><span>Available to Sell:</span><span>"+availToSell.toLocaleString()+"</span></div></div>"}document.getElementById("modalTotal").innerHTML=modalTotalHtml;document.getElementById("modalNote").value="";document.getElementById("modalPickBtn").style.display="none";currentModalProductId=null;window.currentGroupVariants=variants;document.getElementById("modal").classList.add("active");loadSalesHistory(baseStyle)}';
