...
function getPDFHTML(selection, products, options) {
    ...
    var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>' + (selection.name || 'Product Selection') + ' - Mark Edwards Apparel</title><style>@media print{@page{margin:0.5in;size:letter}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;padding:20px;background:white}.header{text-align:center;margin-bottom:30px;padding-bottom:20px;border-bottom:2px solid #1e3a5f}.header h1{font-size:24px;margin-bottom:5px;color:#1e3a5f}.header p{color:#666}.product-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:24px}.product-card{border:1px solid #ddd;border-radius:8px;overflow:hidden;page-break-inside:avoid}.product-image{height:320px;background:#f5f5f5;display:flex;align-items:center;justify-content:center;padding:10px}.product-image img{max-width:100%;max-height:100%;object-fit:contain}.product-info{padding:16px}.product-name{font-size:16px;font-weight:bold;margin-bottom:4px;color:#1e3a5f}.product-style{font-size:12px;color:#666;margin-bottom:10px}.color-row{display:flex;justify-content:space-between;font-size:12px;padding:3px 0}.total-row{margin-top:10px;padding-top:10px;border-top:1px solid #eee;font-weight:bold;display:flex;justify-content:space-between;font-size:14px}.note-box{margin-top:12px;padding:10px;background:#f0f7ff;border-radius:6px;border-left:3px solid #0088c2;font-size:12px;color:#333}.note-label{font-weight:bold;color:#0088c2;margin-bottom:4px}.footer{margin-top:30px;text-align:center;color:#666;font-size:12px}.print-btn{position:fixed;top:20px;right:20px;padding:10px 20px;background:#1e3a5f;color:white;border:none;border-radius:4px;cursor:pointer;font-size:14px}@media print{.print-btn{display:none}}</style></head><body>';
    ...
    for (var i = 0; i < products.length; i++) {
        ...
        if (!hideQuantities) {
            for (var j = 0; j < cols.length; j++) {
                ...
                colHtml += '<div class="color-row"><span>' + cols[j].color_name + '</span><span>' + availNow.toLocaleString() + ' / ' + lts.toLocaleString() + '</span></div>';
            }
        }
        ...
        var totalHtml = '';
        if (!hideQuantities) {
            totalHtml = '<div class="total-row"><span>Total (Now / LTS)</span><span>' + totAvailNow.toLocaleString() + ' / ' + totLts.toLocaleString() + '</span></div>';
        }
        var noteHtml = '';
...
function getHTML() {
    ...
    html += '.login-page{min-height:100vh;display:flex;align-items:center;justify-content:center;background:#fbfbfd}.login-box{background:white;padding:3rem;border-radius:18px;box-shadow:0 4px 40px rgba(0,0,0,0.06);width:100%;max-width:380px}.login-box h1{margin-bottom:2rem;font-size:1.75rem;text-align:center;font-weight:600;color:#1e3a5f;letter-spacing:-0.02em}';
    ...
    html += '.color-dropdown{position:absolute;top:100%;left:0;background:rgba(255,255,255,0.98);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:none;border-radius:12px;box-shadow:0 4px 24px rgba(0,0,0,0.15);padding:0.5rem;z-index:100;max-height:300px;overflow-y:auto;min-width:180px}.color-dropdown.hidden{display:none}.color-option{display:block;padding:0.625rem 1rem;cursor:pointer;border-radius:8px;font-size:0.875rem;color:#1e3a5f}.color-option:hover{background:rgba(0,136,194,0.1);color:#0088c2}.color-option.active{background:#0088c2;color:white}';
    ...
    html += '.product-card.selected{box-shadow:0 0 0 2px #1e3a5f}';
    ...
    html += '.view-controls{display:flex;align-items:center;gap:2rem;margin-bottom:0;padding:1rem 2.5rem;background:#fff;flex-wrap:wrap;position:sticky;top:106px;z-index:80;border-bottom:1px solid rgba(0,0,0,0.06)}.view-controls.compact{padding:0.625rem 2.5rem;top:92px}.view-controls label{font-weight:500;color:#6e6e73;font-size:0.8125rem}';
    ...
    html += '.selection-bar{position:fixed;bottom:0;left:0;right:0;background:rgba(255,255,255,0.9);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);padding:1rem 2rem;box-shadow:0 -1px 0 rgba(0,0,0,0.04);display:flex;justify-content:space-between;align-items:center;z-index:100;transform:translateY(100%);transition:transform 0.3s}.selection-bar.visible{transform:translateY(0)}';
    ...
    html += '<div id="mainApp" class="hidden"><header class="header"><h1>Mark Edwards Apparel Product Catalog</h1><div class="search-box"><input type="text" id="searchInput" placeholder="Search products..."><button id="clearSearchBtn" style="margin-left:0.5rem;padding:0.5rem 0.75rem;border:1px solid #ddd;background:#f5f5f5;border-radius:4px;cursor:pointer">Clear</button><div id="aiSearchIndicator" class="ai-search-indicator hidden">ðŸ¤– AI-enhanced search active</div></div><div class="header-right"><div class="user-menu-wrapper" style="position:relative"><button class="btn btn-secondary" id="userMenuBtn" style="display:flex;align-items:center;gap:0.5rem"><span id="userInfo">Welcome</span> â–¾</button><div id="userMenu" class="user-menu hidden"><button class="user-menu-item" id="changePinBtn">ðŸ”‘ Change PIN</button><button class="user-menu-item" id="logoutBtn">ðŸšª Sign Out</button></div></div><button class="btn btn-secondary" id="helpBtn">Help</button><button class="btn btn-secondary" id="historyBtn">History</button><button class="btn btn-secondary" id="adminBtn" style="display:none">Admin</button></div></header>';
    ...
    html += '<div id="system2Tab" class="tab-content"><div id="systemHealthContent"><p>Loading system health data...</p></div><button class="btn btn-secondary" id="refreshSystemBtn" style="margin-top:1rem">ðŸ”„ Refresh</button></div></div>';
    
    html += '<div class="stats"><div><div class="stat-value" id="totalStyles">0</div><div class="stat-label">Styles</div></div><div id="availNowStat" class="stat-box stat-active"><div class="stat-value" id="totalAvailNow">0</div><div class="stat-label">ðŸ“¦ Avail Now</div></div><div id="leftToSellStat" class="stat-box"><div class="stat-value" id="totalLeftToSell">0</div><div class="stat-label">ðŸ“Š Left to Sell</div></div><div class="qty-toggle"><button class="qty-toggle-btn active" id="toggleAvailableNow" data-mode="available_now">ðŸ“¦ Available Now</button><button class="qty-toggle-btn" id="toggleLeftToSell" data-mode="left_to_sell">ðŸ“Š Left to Sell</button></div></div>';
    html += '<div class="view-controls"><label>View:</label><button class="size-btn" data-size="list">List</button><button class="size-btn" data-size="small">Small</button><button class="size-btn active" data-size="medium">Medium</button><button class="size-btn" data-size="large">Large</button><div class="feature-toggle active-indicator" id="groupByStyleWrapper"><input type="checkbox" id="groupByStyleToggle" checked><label for="groupByStyleToggle">ðŸŽ¨ Group by Style</label></div><label style="margin-left:1.5rem">Sort:</label><select id="sortSelect" style="padding:0.5rem 0.75rem;border:2px solid #1e3a5f;border-radius:8px;font-size:0.8125rem;background:#1e3a5f;color:white;font-weight:500;cursor:pointer"><option value="name-asc">Name A-Z</option><option value="name-desc">Name Z-A</option><option value="qty-high">Qty Highâ†’Low</option><option value="qty-low">Qty Lowâ†’High</option><option value="newest">Newest First</option></select><div class="qty-filter-group" style="margin-left:1.5rem"><label>Qty:</label><input type="number" id="minQty" placeholder="Min"><span>â€“</span><input type="number" id="maxQty" placeholder="Max"><button id="resetQtyBtn" class="size-btn" style="padding:0.4rem 0.75rem">Reset</button></div><span style="margin-left:auto"></span><button class="select-mode-btn" id="selectModeBtn">ðŸ“¤ Select for Sharing</button></div>';
    html += '<div class="filters"><button class="filter-btn special" data-special="new">New Arrivals</button><button class="filter-btn special" data-special="picks">My Picks</button><button class="filter-btn special" data-special="notes">Has Notes</button><span class="filter-divider"></span><div style="display:inline-flex;position:relative;align-items:center;margin-right:0.5rem"><button class="filter-btn" id="colorFilterBtn" style="font-weight:500">Color: All â–¼</button><button class="filter-btn hidden" id="clearColorBtn" style="margin-left:0.25rem;padding:0.4rem 0.625rem">âœ•</button><div id="colorDropdown" class="color-dropdown hidden"></div></div><span class="filter-divider"></span><span id="categoryFilters"></span></div>';
    ...
    // Help modal
    html += '<div class="help-modal" id="helpModal"><div class="help-content"><button class="modal-close" id="helpClose">&times;</button><h2>ðŸ“– Product Catalog Guide</h2><div class="help-sections">';
    
    // Quick Start
    html += '<div class="help-section"><h3>ðŸš€ Quick Start</h3><div class="help-cards"><div class="help-card"><div class="help-icon">ðŸ”</div><h4>Search</h4><p>Type in the search bar to find products by name, style ID, or visual features (AI-powered)</p></div><div class="help-card"><div class="help-icon">ðŸŽ¨</div><h4>Group by Style</h4><p>Products are grouped by base style showing all color variants together (ON by default)</p></div><div class="help-card"><div class="help-icon">ðŸ“¦</div><h4>Quantity Views</h4><p>Toggle between "Available Now" (ready to ship) and "Left to Sell" (including incoming)</p></div><div class="help-card"><div class="help-icon">ðŸ“¤</div><h4>Share</h4><p>Select products and share via email, text, or PDF with your customers</p></div></div></div>';
    
    // Quantity Toggle
    html += '<div class="help-section"><h3>ðŸ“¦ Available Now vs Left to Sell</h3><table class="help-table"><tr><td class="help-feature">ðŸ“¦ <strong>Available Now</strong></td><td>Inventory that is currently in stock and ready to ship immediately. Use this when discussing what you can deliver TODAY.</td></tr><tr><td class="help-feature">ðŸ“Š <strong>Left to Sell</strong></td><td>Available Now PLUS uncommitted incoming inventory. Use this for seasonal planning and future commitments.</td></tr><tr><td class="help-feature">ðŸ”˜ <strong>Toggle Button</strong></td><td>Click the toggle buttons in the stats bar to switch views. The selected quantity type is highlighted in green. All filtering, sorting, and product displays update based on your selection.</td></tr></table></div>';
    
    // Group by Style
    html += '<div class="help-section"><h3>ðŸŽ¨ Group by Style</h3><p>When "Grouped by Style âœ“" is active (default), products with the same base style but different colors are shown as a single card with a color count badge (e.g., "5 colors").</p><table class="help-table"><tr><td class="help-feature"><strong>Grouped View</strong></td><td>Shows one card per base style with total quantities across all colors. Click to see all color variants with swatches.</td></tr><tr><td class="help-feature"><strong>Ungrouped View</strong></td><td>Uncheck the toggle to see each color variant as a separate card. Useful when you need to find specific colors.</td></tr></table></div>';
    
    // Features Guide
    html += '<div class="help-section"><h3>âœ¨ Features Guide</h3><table class="help-table"><tr><td class="help-feature">ðŸ” <strong>Search Bar</strong></td><td>Search by style ID, product name, or descriptive terms like "striped hoodie" or "floral dress". Multi-word searches find products matching ALL terms.</td></tr><tr><td class="help-feature">ðŸ“‚ <strong>Category Filter</strong></td><td>Click category buttons (Sweater, Dress, etc.) to show only that category. Click "All" to reset.</td></tr><tr><td class="help-feature">ðŸŽ¨ <strong>Color Filter</strong></td><td>Click "Color: All â–¼" dropdown to filter by specific colors. Click âœ• to clear.</td></tr><tr><td class="help-feature">ðŸ“Š <strong>Sort Options</strong></td><td>Sort by Name (A-Z, Z-A), Quantity (High/Low), or Newest arrivals. Sorting uses the selected quantity mode.</td></tr><tr><td class="help-feature">ðŸ“ <strong>View Options</strong></td><td>Switch between List, Small, Medium, and Large tile views.</td></tr><tr><td class="help-feature">ðŸ”¢ <strong>Quantity Filter</strong></td><td>Set Min/Max quantity to find products within a specific inventory range. Click "Reset" to clear.</td></tr><tr><td class="help-feature">ðŸ†• <strong>New Arrivals</strong></td><td>Click "New Arrivals" to see products added in the most recent data import.</td></tr></table></div>';
    
    // Product Details & Sales History
    html += '<div class="help-section"><h3>ðŸ“‹ Product Details & Sales History</h3><p>Click any product to open the detail modal with:</p><table class="help-table"><tr><td class="help-feature"><strong>Color Breakdown</strong></td><td>See Available Now and Left to Sell quantities for each color variant.</td></tr><tr><td class="help-feature"><strong>Color Swatches</strong></td><td>In grouped view, click color swatches to switch the displayed image.</td></tr><tr><td class="help-feature">ðŸ“ˆ <strong>Sales History Tab</strong></td><td>View past invoices and open sales orders for this style from Zoho Books, including customer names, quantities, and dollar amounts.</td></tr><tr><td class="help-feature">ðŸ“¦ <strong>Import POs Tab</strong></td><td>See purchase orders (incoming inventory) for this style.</td></tr></table></div>';
    
    // My Picks & Notes
    html += '<div class="help-section"><h3>ðŸ’¾ Save & Organize</h3><table class="help-table"><tr><td class="help-feature">â™¥ <strong>My Picks</strong></td><td>Click the heart icon on any product to save it to your personal favorites. Click "My Picks" filter to see only saved items. Your picks are saved to your account.</td></tr><tr><td class="help-feature">ðŸ“ <strong>Notes</strong></td><td>Click a product to open it, then add private notes in the "My Notes" section. Products with notes show a ðŸ“ badge. Click "Has Notes" filter to find them.</td></tr></table></div>';
    
    // Sharing Guide
    html += '<div class="help-section"><h3>ðŸ“¤ Sharing Products</h3><div class="help-steps"><div class="help-step"><div class="step-num">1</div><div class="step-content"><strong>Enter Selection Mode</strong><p>Click "ðŸ“¤ Select for Sharing" button in the view controls</p></div></div><div class="help-step"><div class="step-num">2</div><div class="step-content"><strong>Select Products</strong><p>Click products to select them (green checkmark appears). Click again to deselect.</p></div></div><div class="help-step"><div class="step-num">3</div><div class="step-content"><strong>Share / Download</strong><p>Click "Share / Download" in the bottom bar that appears</p></div></div><div class="help-step"><div class="step-num">4</div><div class="step-content"><strong>Choose Method</strong><p>ðŸ“§ Email Link, ðŸ’¬ Text Link, ðŸ”— Copy Link, or ðŸ“„ Download PDF</p></div></div></div><p style="margin-top:1rem;color:#666"><strong>Note:</strong> Shared links are view-only. Customers see product images and quantities but cannot place orders directly. Sales history is NOT shown on shared links.</p></div>';
    
    // Keyboard Shortcuts
    html += '<div class="help-section"><h3>âŒ¨ï¸ Keyboard Shortcuts</h3><table class="help-table"><tr><td><kbd>â†</kbd> <kbd>â†’</kbd> <kbd>â†‘</kbd> <kbd>â†“</kbd></td><td>Navigate between products</td></tr><tr><td><kbd>Enter</kbd></td><td>Open selected product / Toggle selection in selection mode</td></tr><tr><td><kbd>Space</kbd></td><td>Toggle selection (in selection mode)</td></tr><tr><td><kbd>Esc</kbd></td><td>Close any open modal</td></tr></table></div>';
    
    // AI Search
    html += '<div class="help-section"><h3>ðŸ¤– AI-Enhanced Search</h3><p>When you see "ðŸ¤– AI-enhanced search active" below the search bar, the catalog has been analyzed by AI to understand what\'s in each product image.</p><p><strong>This means you can search by:</strong></p><ul><li>Garment type: "cardigan", "hoodie", "romper", "jogger"</li><li>Style: "casual", "bohemian", "preppy", "streetwear"</li><li>Pattern: "striped", "floral", "graphic print", "heart print", "tie-dye"</li><li>Features: "v-neck", "button-front", "cropped", "oversized", "zip-up"</li><li>Neckline: "crew neck", "turtleneck", "mock neck", "off-shoulder"</li><li>Combinations: "striped cardigan buttons" or "oversized hoodie graphic"</li></ul><p style="margin-top:0.5rem;color:#666">The AI analyzes product images when data is imported. Search matches both CSV data AND AI-detected visual features.</p></div>';
    
    // History Panel
    html += '<div class="help-section"><h3>ðŸ“Š History Panel</h3><p>Click "History" in the header to access:</p><table class="help-table"><tr><td class="help-feature"><strong>Sharing History</strong></td><td>See all shared selections you\'ve created, with links to view or copy them again.</td></tr><tr><td class="help-feature"><strong>Data Freshness</strong></td><td>Check when inventory data was last updated (CSV import or Zoho sync).</td></tr><tr><td class="help-feature"><strong>Sync History</strong></td><td>View history of all data syncs and imports with status and record counts.</td></tr></table></div>';
    
    // FAQ
    html += '<div class="help-section"><h3>â“ FAQ</h3><div class="faq-item"><strong>Q: What\'s the difference between Available Now and Left to Sell?</strong><p>A: "Available Now" is what you can ship today. "Left to Sell" includes Available Now plus incoming inventory that hasn\'t arrived yet - use this for seasonal planning.</p></div><div class="faq-item"><strong>Q: Why do I see "546 groups" instead of "1110 Styles"?</strong><p>A: When "Group by Style" is ON, products with the same base style but different colors are combined into one group. Toggle it OFF to see individual style/color combinations.</p></div><div class="faq-item"><strong>Q: Why can\'t I find a product I know exists?</strong><p>A: Check if filters are applied (category, color, quantity range). Try clearing the search and filters, or toggle "Group by Style" off to see all variants.</p></div><div class="faq-item"><strong>Q: Are my picks and notes visible to others?</strong><p>A: No, your picks and notes are private to your account.</p></div><div class="faq-item"><strong>Q: Can customers see sales history on shared links?</strong><p>A: No, sales history is only visible to logged-in users. Shared links show products and quantities only.</p></div><div class="faq-item"><strong>Q: How current is the inventory data?</strong><p>A: Check History â†’ Data Freshness to see when data was last updated. Sales history is cached and refreshes automatically.</p></div></div>';
    ...
    html += 'document.getElementById("clearColorBtn").addEventListener("click",function(){colorFilter=null;document.getElementById("colorFilterBtn").textContent="Color: All â–¼";document.getElementById("clearColorBtn").classList.add("hidden");renderProducts()});';
    ...
    html += 'document.getElementById("copyLinkBtn").addEventListener("click",function(){navigator.clipboard.writeText(currentShareUrl).then(function(){var btn=document.getElementById("copyLinkBtn");btn.textContent="âœ“ Copied!";setTimeout(function(){btn.textContent="ðŸ”— Copy Link"},2000)})});';
    ...
    html += 'function loadSystemHealth(){fetch("/api/system-health").then(function(r){return r.json()}).then(function(d){if(!d.success){document.getElementById("systemHealthContent").innerHTML="<p class=\\"error\\">Error loading system health</p>";return}var h="<div class=\\"system-health-grid\\">";h+="<div class=\\"health-card\\"><h4>ðŸ“Š Database</h4><div class=\\"health-stats\\">";h+="<div class=\\"health-row\\"><span>Total Size:</span><strong>"+d.database.totalSize+"</strong></div>";h+="<div class=\\"health-row\\"><span>Products:</span><strong>"+d.database.tables.products.toLocaleString()+"</strong></div>";h+="<div class=\\"health-row\\"><span>Color Variants:</span><strong>"+d.database.tables.product_colors.toLocaleString()+"</strong></div>";h+="<div class=\\"health-row\\"><span>Users:</span><strong>"+d.database.tables.users+"</strong></div>";h+="<div class=\\"health-row\\"><span>Shares Created:</span><strong>"+d.database.tables.selections+"</strong></div>";h+="<div class=\\"health-row\\"><span>User Picks:</span><strong>"+d.database.tables.user_picks+"</strong></div>";h+="<div class=\\"health-row\\"><span>User Notes:</span><strong>"+d.database.tables.user_notes+"</strong></div>";h+="</div></div>";h+="<div class=\\"health-card\\"><h4>ðŸ¤– AI Analysis</h4><div class=\\"health-stats\\">";h+="<div class=\\"health-row\\"><span>Products Analyzed:</span><strong>"+d.database.productsWithAI.toLocaleString()+"</strong></div>";h+="<div class=\\"health-row\\"><span>Pending Analysis:</span><strong>"+d.database.productsWithoutAI.toLocaleString()+"</strong></div>";var aiPct=d.database.tables.products>0?Math.round(d.database.productsWithAI/d.database.tables.products*100):0;h+="<div class=\\"health-row\\"><span>Coverage:</span><strong>"+aiPct+"%</strong></div>";h+="<div class=\\"health-row\\"><span>Anthropic API:</span><strong class=\\"status-"+(d.apiStatus.anthropicConfigured?"ok":"warn")+"\\">"+(d.apiStatus.anthropicConfigured?"âœ“ Configured":"âœ— Not Set")+"</strong></div>";h+="</div></div>";h+="<div class=\\"health-card\\"><h4>ðŸ”— Integrations</h4><div class=\\"health-stats\\">";h+="<div class=\\"health-row\\"><span>Zoho Configured:</span><strong class=\\"status-"+(d.apiStatus.zohoConfigured?"ok":"warn")+"\\">"+(d.apiStatus.zohoConfigured?"âœ“ Yes":"âœ— No")+"</strong></div>";h+="<div class=\\"health-row\\"><span>Zoho Connected:</span><strong class=\\"status-"+(d.apiStatus.zohoConnected?"ok":"warn")+"\\">"+(d.apiStatus.zohoConnected?"âœ“ Yes":"âœ— No")+"</strong></div>";h+="<div class=\\"health-row\\"><span>Last Sync:</span><strong>"+(d.activity.lastSuccessfulSync?new Date(d.activity.lastSuccessfulSync).toLocaleDateString():"Never")+"</strong></div>";h+="<div class=\\"health-row\\"><span>Last Sync Records:</span><strong>"+(d.activity.lastSyncRecords||"-")+"</strong></div>";h+="</div></div>";h+="<div class=\\"health-card\\"><h4>âš¡ Server</h4><div class=\\"health-stats\\">";h+="<div class=\\"health-row\\"><span>Uptime:</span><strong>"+d.server.uptime+"</strong></div>";h+="<div class=\\"health-row\\"><span>Memory Used:</span><strong>"+d.server.memoryUsed+"</strong></div>";h+="<div class=\\"health-row\\"><span>Memory Total:</span><strong>"+d.server.memoryTotal+"</strong></div>";h+="<div class=\\"health-row\\"><span>Node.js:</span><strong>"+d.server.nodeVersion+"</strong></div>";h+="<div class=\\"health-row\\"><span>Platform:</span><strong>"+d.server.platform+"</strong></div>";h+="</div></div>";h+="<div class=\\"health-card\\"><h4>ðŸ“ˆ Activity (7 days)</h4><div class=\\"health-stats\\">";h+="<div class=\\"health-row\\"><span>Users with Picks:</span><strong>"+d.activity.activeUsers+"</strong></div>";h+="<div class=\\"health-row\\"><span>Shares Created:</span><strong>"+d.activity.sharesLast7Days+"</strong></div>";h+="<div class=\\"health-row\\"><span>Data Syncs:</span><strong>"+d.activity.syncsLast7Days+"</strong></div>";h+="</div></div>";h+="</div>";h+="<p style=\\"margin-top:1rem;font-size:0.8rem;color:#999\\">Last checked: "+new Date(d.timestamp).toLocaleString()+"</p>";document.getElementById("systemHealthContent").innerHTML=h}).catch(function(e){document.getElementById("systemHealthContent").innerHTML="<p class=\\"error\\">Error: "+e.message+"</p>"})}';
