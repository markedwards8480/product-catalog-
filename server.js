...
function getHTML() {
    ...
    html += 'function renderFreshnessDetail(d){var list=document.getElementById("freshnessDetailList");if(!list)return;var h="";var types=["lts_inventory","lts_inventory_sizes","avail_now_inventory","avail_now_inventory_sizes","sales_orders_pos"];var detailMap={};if(d.detail){d.detail.forEach(function(item){detailMap[item.data_type]=item})}types.forEach(function(t){var label=freshnessTypeLabels[t]||t;var item=detailMap[t];var isSizeType=t.indexOf("sizes")!==-1;if(item){var dt=new Date(item.last_updated);var hoursSince=(Date.now()-dt.getTime())/(1000*60*60);var staleColor=hoursSince>24?"#e74c3c":hoursSince>12?"#f39c12":"#27ae60";var timeStr=dt.toLocaleDateString()+" "+dt.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});h+="<div style=\"display:flex;justify-content:space-between;align-items:center;padding:0.35rem 0;border-bottom:1px solid #f0f0f0\"><div><span style=\"color:#1e3a5f;font-weight:500\">"+label+"</span>"+(item.record_count?"<span style=\"color:#999;margin-left:0.5rem\">("+item.record_count.toLocaleString()+" rows)</span>":"")+"</div><div style=\"display:flex;align-items:center;gap:0.35rem\"><span style=\"width:6px;height:6px;border-radius:50%;background:"+staleColor+";display:inline-block\"></span><span style=\"color:#666\">"+timeStr+"</span></div></div>"}else{h+="<div style=\"display:flex;justify-content:space-between;align-items:center;padding:0.35rem 0;border-bottom:1px solid #f0f0f0\"><div><span style=\"color:#1e3a5f;font-weight:500\">"+label+"</span></div><div style=\"display:flex;align-items:center;gap:0.35rem\"><span style=\"width:6px;height:6px;border-radius:50%;background:#ccc;display:inline-block\"></span><span style=\"color:#999\">No data</span></div></div>"}});if(d.sizeRows!==undefined){h+="<div style=\"margin-top:0.5rem;padding-top:0.5rem;border-top:1px solid #ddd;color:"+(d.sizeRows>0?"#27ae60":"#e74c3c")+";font-weight:600\">Size Data: "+(d.sizeRows>0?d.sizeRows.toLocaleString()+" rows loaded":"⚠ EMPTY - no size data loaded")+"</div>"}list.innerHTML=h}';
