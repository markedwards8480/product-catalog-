...
function getHTML() {
    ...
    html += 'var el;';
    html += 'el=document.getElementById("modalOrderBtn");if(el)el.addEventListener("click",openOrderRequestModal);';
    html += 'el=document.getElementById("orCancelBtn");if(el)el.addEventListener("click",function(){document.getElementById("orderRequestModal").classList.remove("active")});';
    html += 'el=document.getElementById("orDoneBtn");if(el)el.addEventListener("click",function(){document.getElementById("orderRequestModal").classList.remove("active")});';
    html += 'el=document.getElementById("orderRequestModal");if(el)el.addEventListener("click",function(e){if(e.target.id==="orderRequestModal")e.target.classList.remove("active")});';
    html += 'el=document.getElementById("myOrdersFilter");if(el)el.addEventListener("change",loadMyOrders);';
    html += 'el=document.getElementById("adminOrdersFilter");if(el)el.addEventListener("change",loadAdminOrders);';
    html += 'el=document.getElementById("orSubmitBtn");if(el)el.addEventListener("click",submitOrderRequest);';
    html += '},100);';
    ...
    html += 'function submitOrderRequest(){var customer=document.getElementById("orCustomer").value;if(!customer){alert("Please select a customer");return}var sizes=[];var total=0;document.querySelectorAll(".or-size-input").forEach(function(inp){var qty=parseInt(inp.value)||0;if(qty>0){sizes.push({size:inp.getAttribute("data-size"),qty:qty});total+=qty}});if(total===0){alert("Enter quantities for at least one size");return}var btn=document.getElementById("orSubmitBtn");btn.disabled=true;btn.textContent="Submitting...";var style=document.getElementById("orProductStyle").textContent;var body={customer_name:customer,style_id:style,base_style:style.split("-")[0],product_name:document.getElementById("orProductName").textContent,color_info:(document.getElementById("modalCategory")||{}).textContent||"",size_breakdown:JSON.stringify(sizes),total_qty:total,notes:document.getElementById("orNotes").value.trim()};fetch("/api/order-requests",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){if(d.success){document.getElementById("orderFormView").classList.add("hidden");document.getElementById("orderSuccessView").classList.remove("hidden");document.getElementById("orSuccessNum").textContent=d.order.request_number;loadOrderBadge()}else{alert("Error: "+(d.error||"Unknown"))}btn.disabled=false;btn.textContent="Submit Order Request"}).catch(function(e){alert("Error: "+e.message);btn.disabled=false;btn.textContent="Submit Order Request"})}';
