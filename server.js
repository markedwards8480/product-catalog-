...
function getHTML() {
    ...
    html += 'function loadLoginUsers(){fetch("/api/users/list").then(function(r){return r.json()}).then(function(users){var sel=document.getElementById("loginUserSelect");if(sel){if(users.length===0){showApp("User",null);loadProducts();loadPicks();loadNotes();loadZohoStatus();loadDataFreshness();loadShares();loadHistory();return}users.forEach(function(u){var opt=document.createElement("option");opt.value=u.id;opt.textContent=u.display_name;sel.appendChild(opt)})}})}';
