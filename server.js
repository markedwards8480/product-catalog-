...
function getHTML() {
    ...
    html += 'function showProductModal(id){currentModalProductId=id;var pr=products.find(function(p){return p.id===id});if(!pr)return;var imgUrl=getImageUrl(pr.image_url);document.getElementById("modalImage").src=imgUrl||"";document.getElementById("modalStyle").textContent=pr.style_id;document.getElementById("modalName").textContent=pr.name;document.getElementById("modalCategory").textContent=pr.category||"";var cols=pr.colors||[];var totNow=0,totLts=0;cols.forEach(function(c){var aNow=c.available_now||c.available_qty||0;var lts=c.left_to_sell||0;totNow+=aNow;totLts+=lts});var ch="";if(cols.length>1){ch="<div style=\\"display:grid;grid-template-columns:1fr auto auto;gap:1rem;font-weight:bold;padding-bottom:0.5rem;margin-bottom:0.5rem;border-bottom:1px solid #ddd;font-size:0.8rem;color:#666\\"><span>Color</span><span style=\\"text-align:right\\">Avail Now</span><span style=\\"text-align:right\\">Left to Sell</span></div>";cols.forEach(function(c){var aNow=c.available_now||c.available_qty||0;var lts=c.left_to_sell||0;ch+="<div class=\\"color-row\\" style=\\"display:grid;grid-template-columns:1fr auto auto;gap:1rem\\"><span>"+c.color_name+"</span><span style=\\"text-align:right\\">"+aNow.toLocaleString()+"</span><span style=\\"text-align:right;color:#666\\">"+lts.toLocaleString()+"</span></div>"})}document.getElementById("modalColors").innerHTML=ch;document.getElementById("modalTotal").innerHTML="<span style=\\"margin-right:2rem\\">📦 Now: "+totNow.toLocaleString()+"</span><span>📊 LTS: "+totLts.toLocaleString()+"</span>";document.getElementById("modalNote").value=userNotes[id]||"";var isPicked=userPicks.indexOf(id)!==-1;document.getElementById("modalPickBtn").textContent=isPicked?"♥ In My Picks":"♡ Add to My Picks";document.getElementById("modal").classList.add("active");loadSalesHistory(pr.style_id)}';
    ...
    html += 'function showGroupModal(baseStyle,variants){var totNow=0,totLts=0;variants.forEach(function(v){(v.colors||[]).forEach(function(c){totNow+=(c.available_now||c.available_qty||0);totLts+=(c.left_to_sell||0)})});document.getElementById("modalStyle").textContent=baseStyle;document.getElementById("modalName").textContent=variants[0].name.replace(variants[0].style_id,baseStyle);document.getElementById("modalCategory").textContent=variants[0].category||"";var imgUrl=getImageUrl(variants[0].image_url);document.getElementById("modalImage").src=imgUrl||"";var swatchHtml="<div style=\\"display:flex;gap:0.5rem;flex-wrap:wrap;margin-bottom:1rem\\">";variants.forEach(function(v,i){var vImg=getImageUrl(v.image_url);var colorCode=v.style_id.split("-")[1]||"";var colorName=(v.colors&&v.colors[0])?v.colors[0].color_name:colorCode;swatchHtml+="<div class=\\"color-swatch"+(i===0?" active":"")+"\\" data-idx=\\""+i+"\\" style=\\"padding:0.5rem 0.75rem;border:2px solid "+(i===0?"#1a3b5d":"#ddd")+";border-radius:4px;cursor:pointer;font-size:0.75rem;background:"+(i===0?"#f0f4f8":"#fff")+"\\" onclick=\\"switchVariantImage("+i+",\'"+baseStyle+"\')\\">"+colorName+"</div>"});swatchHtml+="</div>";var ch="<div style=\\"display:grid;grid-template-columns:1fr auto auto;gap:1rem;font-weight:bold;padding-bottom:0.5rem;margin-bottom:0.5rem;border-bottom:1px solid #ddd;font-size:0.8rem;color:#666\\"><span>Color Variant</span><span style=\\"text-align:right\\">Avail Now</span><span style=\\"text-align:right\\">Left to Sell</span></div>";variants.forEach(function(v){var vNow=0,vLts=0;(v.colors||[]).forEach(function(c){vNow+=(c.available_now||c.available_qty||0);vLts+=(c.left_to_sell||0)});var colorName=(v.colors&&v.colors[0])?v.colors[0].color_name:v.style_id;ch+="<div class=\\"color-row\\" style=\\"display:grid;grid-template-columns:1fr auto auto;gap:1rem\\"><span>"+colorName+"</span><span style=\\"text-align:right\\">"+vNow.toLocaleString()+"</span><span style=\\"text-align:right;color:#666\\">"+vLts.toLocaleString()+"</span></div>"});document.getElementById("modalColors").innerHTML=swatchHtml+ch;document.getElementById("modalTotal").innerHTML="<span style=\\"margin-right:2rem\\">📦 Now: "+totNow.toLocaleString()+"</span><span>📊 LTS: "+totLts.toLocaleString()+"</span>";document.getElementById("modalNote").value="";document.getElementById("modalPickBtn").style.display="none";document.getElementById("modalCompareBtn").style.display="none";currentModalProductId=null;window.currentGroupVariants=variants;document.getElementById("modal").classList.add("active");loadSalesHistory(baseStyle)}';
