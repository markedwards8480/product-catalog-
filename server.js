...
function getHTML() {
    ...
    html += '.chat-bubble{position:fixed;bottom:24px;right:24px;width:56px;height:56px;background:#1e3a5f;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,0.16);z-index:999;transition:all 0.2s}.chat-bubble:hover{transform:scale(1.08);background:#1e3a5f}.chat-bubble svg{width:26px;height:26px;fill:white}';
    ...
    html += 'var currentSalesFilter="all";var currentSalesHistory=[];function loadSalesHistory(styleId){currentSalesFilter="all";document.getElementById("salesHistoryLoading").textContent="(loading...)";document.getElementById("salesHistorySummary").innerHTML="";document.getElementById("salesHistoryFilter").innerHTML="";document.getElementById("salesHistoryList").innerHTML="<div style=\\"color:#666;padding:0.5rem\\">Loading...</div>";fetch("/api/sales-history/"+encodeURIComponent(styleId)).then(function(r){return r.json()}).then(function(d){document.getElementById("salesHistoryLoading").textContent="";if(!d.success){document.getElementById("salesHistoryList").innerHTML="<div style=\\"color:#999;padding:0.5rem\\">Unable to load</div>";return}currentSalesHistory=d.history;var sum=d.summary;var invDollars=sum.totalInvoicedDollars?"$"+sum.totalInvoicedDollars.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"";var openDollars=sum.totalOpenOrdersDollars?"$"+sum.totalOpenOrdersDollars.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"";var poDollars=sum.totalPODollars?"$"+sum.totalPODollars.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"";document.getElementById("salesHistorySummary").innerHTML="<div onclick=\\"filterSalesHistory(\'invoiced\')\\" style=\\"padding:0.5rem 0.75rem;background:#e8f5e9;border-radius:6px;cursor:pointer;border:2px solid transparent\\" class=\\"sales-tile\\" data-filter=\\"invoiced\\"><div style=\\"font-weight:bold;color:#2e7d32\\">"+sum.totalInvoiced.toLocaleString()+"</div><div style=\\"font-size:0.7rem;color:#666\\">Invoiced ("+sum.invoiceCount+")</div>"+(invDollars?"<div style=\\"font-size:0.75rem;color:#2e7d32\\">"+invDollars+"</div>":"")+"</div><div onclick=\\"filterSalesHistory(\'open\')\\" style=\\"padding:0.5rem 0.75rem;background:#fff3e0;border-radius:6px;cursor:pointer;border:2px solid transparent\\" class=\\"sales-tile\\" data-filter=\\"open\\"><div style=\\"font-weight:bold;color:#ef6c00\\">"+sum.totalOpenOrders.toLocaleString()+"</div><div style=\\"font-size:0.7rem;color:#666\\">Open SO ("+sum.openOrderCount+")</div>"+(openDollars?"<div style=\\"font-size:0.75rem;color:#ef6c00\\">"+openDollars+"</div>":"")+"</div><div onclick=\\"filterSalesHistory(\'po\')\\" style=\\"padding:0.5rem 0.75rem;background:#e3f2fd;border-radius:6px;cursor:pointer;border:2px solid transparent\\" class=\\"sales-tile\\" data-filter=\\"po\\"><div style=\\"font-weight:bold;color:#1565c0\\">"+(sum.totalPO||0).toLocaleString()+"</div><div style=\\"font-size:0.7rem;color:#666\\">Import PO ("+(sum.poCount||0)+")</div>"+(poDollars?"<div style=\\"font-size:0.75rem;color:#1565c0\\">"+poDollars+"</div>":"")+"</div>";renderSalesHistoryList(d.history)}).catch(function(err){document.getElementById("salesHistoryLoading").textContent="";document.getElementById("salesHistoryList").innerHTML="<div style=\\"color:#999\\">Error: "+err.message+"</div>"})}';
