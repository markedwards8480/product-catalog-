...
function getHTML() {
    ...
    // Apple-style base
    html += '*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","SF Pro Text",sans-serif;background:#fff;color:#1d1d1f;font-weight:400;-webkit-font-smoothing:antialiased}';
    html += '.login-page{min-height:100vh;display:flex;align-items:center;justify-content:center;background:#fbfbfd}.login-box{background:white;padding:3rem;border-radius:18px;box-shadow:0 4px 40px rgba(0,0,0,0.06);width:100%;max-width:380px}.login-box h1{margin-bottom:2rem;font-size:1.75rem;text-align:center;font-weight:600;color:#1d1d1f;letter-spacing:-0.02em}';
    ...
    // Apple-style buttons
    html += '.btn{padding:0.75rem 1.5rem;border:none;border-radius:980px;cursor:pointer;font-size:0.875rem;font-weight:400;transition:all 0.2s;letter-spacing:-0.01em}.btn-primary{background:#0071e3;color:white}.btn-primary:hover{background:#0077ed}.btn-secondary{background:transparent;color:#0071e3;padding:0.5rem 1rem}.btn-secondary:hover{background:rgba(0,113,227,0.04)}.btn-danger{background:#ff3b30;color:white}.btn-danger:hover{background:#ff453a}.btn-success{background:#34c759;color:white}.btn-success:hover{background:#30d158}';
    ...
    html += '.ai-search-indicator{position:absolute;top:100%;left:0;font-size:0.7rem;color:#34c759;margin-top:0.25rem;white-space:nowrap}.ai-search-indicator.hidden{display:none}';
    html += '.main{max-width:1600px;margin:0 auto;padding:2rem}';
    // Admin panel
    html += '.admin-panel{background:#fff;padding:2rem;border-radius:18px;margin-bottom:2rem;border:1px solid rgba(0,0,0,0.04)}.admin-panel h2{margin-bottom:1.5rem;font-weight:600;color:#1d1d1f;font-size:1.25rem;letter-spacing:-0.02em}';
    html += '.tabs{display:flex;gap:0.25rem;margin-bottom:1.5rem;flex-wrap:wrap}.tab{padding:0.625rem 1rem;border:none;background:transparent;cursor:pointer;border-radius:980px;color:#1d1d1f;font-size:0.875rem;font-weight:400;transition:all 0.2s}.tab:hover{background:rgba(0,0,0,0.04)}.tab.active{background:#1d1d1f;color:white}';
    html += '.upload-area{border:1px dashed #d2d2d7;padding:3rem;text-align:center;border-radius:12px;margin-bottom:1rem;background:#fbfbfd}.upload-area input{display:none}.upload-area label{color:#0071e3;cursor:pointer;font-weight:400}';
    // Stats bar - Apple minimal style but punchier
    html += '.stats{display:flex;gap:3rem;margin-bottom:0;padding:1.5rem 2.5rem;background:#fff;align-items:center;flex-wrap:wrap;position:sticky;top:48px;z-index:90;border-bottom:1px solid rgba(0,0,0,0.06)}.stats.compact{padding:0.75rem 2.5rem;top:44px}.stat-value{font-size:2rem;font-weight:600;color:#1d1d1f;letter-spacing:-0.03em}.stat-label{color:#6e6e73;font-size:0.75rem;font-weight:500;text-transform:uppercase;letter-spacing:0.02em;margin-top:0.125rem}.stat-box{padding:0;transition:all 0.2s ease}.stat-box.stat-active .stat-value{color:#1d1d1f}.stat-box:not(.stat-active){opacity:0.35}.feature-toggle.active-indicator{background:transparent}';
    // Qty toggle - Apple pill style
    html += '.qty-toggle{display:flex;background:#f5f5f7;border-radius:980px;padding:3px;margin-left:auto}.qty-toggle-btn{padding:0.5rem 1.25rem;border:none;background:transparent;cursor:pointer;font-size:0.8125rem;font-weight:500;border-radius:980px;transition:all 0.2s;color:#6e6e73}.qty-toggle-btn.active{background:white;color:#1d1d1f;box-shadow:0 1px 4px rgba(0,0,0,0.1)}.qty-toggle-btn:hover:not(.active){color:#1d1d1f}';
    ...
    // Special filters (New Arrivals, My Picks, Has Notes) - BOLDER, more prominent
    html += '.filter-btn.special{padding:0.5rem 1rem;border:none;background:transparent;border-radius:980px;cursor:pointer;color:#1d1d1f;font-size:0.9375rem;font-weight:600;transition:all 0.2s;margin-right:0.5rem}.filter-btn.special:hover{background:rgba(0,0,0,0.04)}.filter-btn.special.active{background:#1d1d1f;color:white}';
    // Divider between special filters and categories
    html += '.filter-divider{width:1px;height:24px;background:#d2d2d7;margin:0 1rem}';
    // Category filters - lighter weight, smaller
    html += '.filter-btn{padding:0.4rem 0.875rem;border:none;background:transparent;border-radius:980px;cursor:pointer;color:#6e6e73;font-size:0.8125rem;font-weight:400;transition:all 0.2s}.filter-btn:hover{background:rgba(0,0,0,0.04);color:#1d1d1f}.filter-btn.active{background:#1d1d1f;color:white;font-weight:500}';
    // Color dropdown - Apple clean style
    html += '.color-dropdown{position:absolute;top:100%;left:0;background:rgba(255,255,255,0.98);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:none;border-radius:12px;box-shadow:0 4px 24px rgba(0,0,0,0.15);padding:0.5rem;z-index:100;max-height:300px;overflow-y:auto;min-width:180px}.color-dropdown.hidden{display:none}.color-option{display:block;padding:0.625rem 1rem;cursor:pointer;border-radius:8px;font-size:0.875rem;color:#1d1d1f}.color-option:hover{background:rgba(0,0,0,0.04)}.color-option.active{background:#0071e3;color:white}';
    // Product grid - Apple spacing, wider
    html += '.product-grid{display:grid;gap:1.5rem;padding:1.5rem 2.5rem}.product-grid.size-small{grid-template-columns:repeat(auto-fill,minmax(220px,1fr))}.product-grid.size-medium{grid-template-columns:repeat(auto-fill,minmax(300px,1fr))}.product-grid.size-large{grid-template-columns:repeat(auto-fill,minmax(400px,1fr))}.product-grid.size-list{display:flex;flex-direction:column;gap:0.5rem}';
    ...
    html += '.color-count-badge{position:absolute;top:12px;right:12px;background:#1d1d1f;color:white;padding:0.25rem 0.625rem;border-radius:980px;font-size:0.7rem;font-weight:500;z-index:5}';
    ...
    html += '.selection-bar{position:fixed;bottom:0;left:0;right:0;background:#1d1d1f;color:white;padding:1rem 2rem;display:flex;justify-content:space-between;align-items:center;z-index:100;transform:translateY(100%);transition:transform 0.3s}.selection-bar.visible{transform:translateY(0)}';
    ...
    html += '.help-cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1rem}.help-card{background:#f5f5f7;border-radius:12px;padding:1.5rem;text-align:center}.help-icon{font-size:2rem;margin-bottom:0.5rem}.help-card h4{margin-bottom:0.5rem;color:#1d1d1f;font-weight:600}.help-card p{font-size:0.875rem;color:#86868b;margin:0}';
    ...
    html += '.help-steps{display:flex;flex-direction:column;gap:1rem}.help-step{display:flex;align-items:flex-start;gap:1rem}.step-num{width:32px;height:32px;background:#1d1d1f;color:white;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:600;flex-shrink:0}.step-content p{margin:0.25rem 0 0;color:#86868b;font-size:0.875rem}';
    html += '.help-section ul{margin:0.5rem 0 0 1.5rem;color:#86868b}.help-section ul li{margin-bottom:0.25rem}';
    html += '.faq-item{margin-bottom:1rem;padding:1rem;background:#f5f5f7;border-radius:12px}.faq-item strong{color:#1d1d1f}.faq-item p{margin:0.5rem 0 0;color:#86868b;font-size:0.875rem}';
    
    // Chat UI CSS - Apple style
    html += '.chat-bubble{position:fixed;bottom:24px;right:24px;width:56px;height:56px;background:#1d1d1f;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,0.16);z-index:999;transition:all 0.2s}.chat-bubble:hover{transform:scale(1.08);background:#0071e3}.chat-bubble svg{width:26px;height:26px;fill:white}';
    ...
    html += 'kbd{background:#f5f5f7;border:none;border-radius:6px;padding:0.25rem 0.5rem;font-family:SF Mono,monospace;font-size:0.8125rem;color:#1d1d1f}';
    ...
    html += '.status-box{padding:1rem;background:#f5f5f7;border-radius:12px;margin-bottom:1rem}.status-item{margin-bottom:0.5rem}.status-label{font-weight:400;color:#86868b}.status-value{color:#1d1d1f}.status-value.connected{color:#34c759}.status-value.disconnected{color:#ff3b30}';
    html += '.system-health-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:1rem}.health-card{background:#f5f5f7;border-radius:12px;padding:1.25rem}.health-card h4{margin:0 0 0.75rem;font-size:0.9375rem;color:#1d1d1f;font-weight:600}.health-stats{display:flex;flex-direction:column;gap:0.5rem}.health-row{display:flex;justify-content:space-between;font-size:0.8125rem}.health-row span{color:#86868b}.health-row strong{color:#1d1d1f}.status-ok{color:#34c759}.status-warn{color:#ff9500}';
    ...
    html += '.qty-filter-group{display:flex;align-items:center;gap:0.5rem}.qty-filter-group input{width:70px;padding:0.4rem 0.625rem;border:1px solid #d2d2d7;border-radius:8px;font-size:0.8125rem;text-align:center}.qty-filter-group input:focus{outline:none;border-color:#0071e3}.qty-filter-group span{color:#86868b;font-size:0.8125rem}';
    html += '.size-btn{padding:0.5rem 1rem;border:none;background:transparent;cursor:pointer;color:#1d1d1f;font-size:0.8125rem;font-weight:500;transition:all 0.2s;border-radius:6px}.size-btn:hover{background:rgba(0,0,0,0.04)}.size-btn.active{background:#1d1d1f;color:white}';
    ...
    html += '.selection-count{font-weight:600;font-size:1rem;color:#1d1d1f}.selection-actions{display:flex;gap:0.5rem}';
    html += '.share-modal{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);display:none;align-items:center;justify-content:center;z-index:1001}.share-modal.active{display:flex}.share-modal-content{background:white;border-radius:18px;padding:2rem;max-width:500px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,0.2)}.share-modal h3{margin-bottom:1rem;font-weight:600;color:#1d1d1f;font-size:1.25rem}.share-modal input{width:100%;padding:0.875rem 1rem;border:none;border-radius:12px;margin-bottom:1rem;background:#f5f5f7}.share-modal-actions{display:flex;gap:0.5rem;justify-content:flex-end}';
    html += '.share-result{margin-top:1rem;padding:1rem;background:#f5f5f7;border-radius:12px}.share-result a{color:#0071e3;word-break:break-all}';
    html += '.share-buttons{display:flex;flex-direction:column;gap:0.75rem}.share-action-btn{display:block;width:100%;padding:1rem;border:none;background:#f5f5f7;border-radius:12px;font-size:1rem;cursor:pointer;text-align:left;text-decoration:none;color:#1d1d1f;transition:all 0.2s}.share-action-btn:hover{background:#ebebed}';
    html += '.select-mode-btn{padding:0.5rem 1rem;border:none;background:transparent;color:#0071e3;border-radius:980px;cursor:pointer;font-weight:400;font-size:0.875rem;transition:all 0.2s}.select-mode-btn:hover{background:rgba(0,113,227,0.04)}.select-mode-btn.active{background:#0071e3;color:white}';
    html += '.feature-toggle{display:inline-flex;align-items:center;margin:0 1rem;background:#f5f5f7;border-radius:980px;padding:3px;border:none}.feature-toggle input{display:none}.feature-toggle label{padding:0.5rem 1rem;border-radius:980px;cursor:pointer;font-size:0.8125rem;font-weight:400;color:#86868b;transition:all 0.2s}.feature-toggle input:checked+label{background:#1d1d1f;color:white}';
    ...
    html += '<div class="view-controls"><label>View:</label><button class="size-btn" data-size="list">List</button><button class="size-btn" data-size="small">Small</button><button class="size-btn active" data-size="medium">Medium</button><button class="size-btn" data-size="large">Large</button><div class="feature-toggle active-indicator" id="groupByStyleWrapper"><input type="checkbox" id="groupByStyleToggle" checked><label for="groupByStyleToggle">🎨 Grouped by Style ✓</label></div><label style="margin-left:1.5rem">Sort:</label><select id="sortSelect" style="padding:0.5rem 0.75rem;border:1px solid #d2d2d7;border-radius:8px;font-size:0.8125rem;background:white"><option value="name-asc">Name A-Z</option><option value="name-desc">Name Z-A</option><option value="qty-high">Qty High→Low</option><option value="qty-low">Qty Low→High</option><option value="newest">Newest First</option></select><div class="qty-filter-group" style="margin-left:1.5rem"><label>Qty:</label><input type="number" id="minQty" placeholder="Min"><span>–</span><input type="number" id="maxQty" placeholder="Max"><button id="resetQtyBtn" class="size-btn" style="padding:0.4rem 0.75rem">Reset</button></div><span style="margin-left:auto"></span><button class="select-mode-btn" id="selectModeBtn">📤 Select for Sharing</button></div>';
