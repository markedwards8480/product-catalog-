...
function getHTML() {
    ...
    html += '.reset-all-btn{padding:0.5rem 1rem;border:none;background:#fee2e2;color:#dc2626;border-radius:980px;cursor:pointer;font-weight:500;font-size:0.8125rem;transition:all 0.2s}.reset-all-btn:hover{background:#fecaca}';
    ...
    html += 'document.getElementById("resetAllFiltersBtn").addEventListener("click",function(){document.getElementById("searchInput").value="";document.getElementById("minQty").value="";document.getElementById("maxQty").value="";currentFilter="all";colorFilter=null;specialFilter=null;minColorsFilter=0;customerStyleFilter=null;currentSort="qty-high";document.getElementById("sortSelect").value="qty-high";document.querySelectorAll(".filter-btn").forEach(function(b){b.classList.remove("active")});document.querySelector(".filter-btn[data-cat=\\"all\\"]").classList.add("active");document.querySelectorAll(".filter-btn[data-special]").forEach(function(b){b.classList.remove("active")});document.getElementById("colorFilterBtn").textContent="Color: All ▼";document.getElementById("clearColorBtn").classList.add("hidden");renderProducts()});';
