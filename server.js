    ...
    for (var i = 0; i < products.length; i++) {
        ...
        if (!hideQuantities) {
            for (var j = 0; j < cols.length; j++) {
                ...
                colHtml += '<div class="color-row"><span>' + cols[j].color_name + '</span><span style="text-align:right"><div>In DC: ' + availNow.toLocaleString() + '</div><div>Coming Soon: ' + lts.toLocaleString() + '</div></span></div>';
            }
        }
        ...
        var totalHtml = '';
        if (!hideQuantities) {
            totalHtml = '<div class="total-row"><span>Total</span><span style="text-align:right"><div>In DC: ' + totAvailNow.toLocaleString() + '</div><div>Coming Soon: ' + totLts.toLocaleString() + '</div></span></div>';
        }
        var noteHtml = '';
